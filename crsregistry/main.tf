...
resource "azurerm_virtual_machine" "crsreg" {
  ...
  storage_data_disk {
    name              = "${var.project}-${var.ocpcrs_registry_module_name}-${format("%02d", count.index)}-p30-disk00"
    caching           = "ReadWrite"
  ...
  storage_data_disk {
    name              = "${var.project}-${var.ocpcrs_registry_module_name}-${format("%02d", count.index)}-p30-disk01"
    caching           = "ReadWrite"
    disk_size_gb      = "${var.gluster_disk_size}"
    create_option     = "Empty"
    lun               = 2
  ...
  storage_data_disk {
    name              = "${var.project}-${var.ocpcrs_registry_module_name}-${format("%02d", count.index)}-p30-disk02"
    caching           = "ReadWrite"
    disk_size_gb      = "${var.gluster_disk_size}"
    create_option     = "Empty"
    lun               = 3
  ...
  storage_data_disk {
    name              = "${var.project}-${var.ocpcrs_registry_module_name}-${format("%02d", count.index)}-p30-disk03"
    caching           = "ReadWrite"
    disk_size_gb      = "${var.gluster_disk_size}"
    create_option     = "Empty"
    lun               = 4
...
resource "azurerm_virtual_machine_extension" "crsregPrep" {
  name                 = "${var.ocpcrs_registry_module_name}Prep-${count.index}"
  location             = "${data.azurerm_resource_group.ocp.location}"
  resource_group_name  = "${data.azurerm_resource_group.ocp.name}"
  virtual_machine_name = "${element(azurerm_virtual_machine.crsreg.*.name, format("%02d", count.index))}"
  publisher            = "Microsoft.Azure.Extensions"
  type                 = "CustomScript"
  type_handler_version = "2.0"
  count                = "${var.crsreg_instance_count}"
  depends_on           = ["azurerm_virtual_machine.crsreg"]
  ...
  protected_settings = <<PROTECTED_SETTINGS
           {
              "commandToExecute": "bash glusterPrep_cri-o.sh \"${var.rhn_user}\" \"${var.rhn_passwd}\" \"${var.rhn_poolid}\" \"${var.crs_poolid}\""
           }
       PROTECTED_SETTINGS
