...
# randomID to be used as part of unique naming throughout project
resource "random_id" "value01" {
  byte_length = 4
}
...
resource "azurerm_storage_container" "node" {
  name                  = "${var.project}-${var.ocpnode_module_name}"
  resource_group_name   = "${data.azurerm_resource_group.ocp.name}"
  storage_account_name  = "${var.storage_account_name}"
  container_access_type = "private"
}
...
resource "local_file" "crsapp-main" {
  ...
  filename = "./output/crsapp-main.tf"
}
...
resource "local_file" "crsregistry-main" {
  ...
  filename = "./output/crsreg-main.tf"
}
