...
resource "azurerm_network_security_group" "infra_nsg" {
  name                = "${var.project}-${var.environment}-infra-nsg"
...
resource "azurerm_storage_account" "infra_storage_account" {
  name                     = "${var.project}${var.environment}infrasa"
  resource_group_name      = "${data.terraform_remote_state.bootstrap.resource_group_name}"
  location                 = "${data.terraform_remote_state.bootstrap.location}"
...
resource "azurerm_storage_account" "registry_storage_account" {
  name                     = "${var.project}${var.environment}regsa"
  resource_group_name      = "${data.terraform_remote_state.bootstrap.resource_group_name}"
  location                 = "${data.terraform_remote_state.bootstrap.location}"
  account_tier             = "${var.os_storage_account_tier}"
  account_replication_type = "${var.storage_account_replication_type}"
...
resource "azurerm_availability_set" "infra" {
  name                = "${var.project}${var.environment}infraavailset"
  resource_group_name = "${data.terraform_remote_state.bootstrap.resource_group_name}"
  location            = "${data.terraform_remote_state.bootstrap.location}"
  managed             = "true"
}
...
resource "azurerm_public_ip" "infra_lb_pip" {
  name                         = "${var.project}${var.environment}infrapip"
  resource_group_name          = "${data.terraform_remote_state.bootstrap.resource_group_name}"
  location                     = "${data.terraform_remote_state.bootstrap.location}"
  public_ip_address_allocation = "Static"
  domain_name_label            = "${var.project}infra${data.terraform_remote_state.bootstrap.random_id}"
}

# ******* INFRA LOAD BALANCER ***********

resource "azurerm_lb" "infra_lb" {
  name                = "${var.project}${var.environment}infralb"
  resource_group_name = "${data.terraform_remote_state.bootstrap.resource_group_name}"
  location            = "${data.terraform_remote_state.bootstrap.location}"
...
resource "azurerm_network_interface" "infra_nic" {
  name                          = "${var.project}-${var.ocpinfra_module_name}-nic${format("%02d", count.index)}"
  location                      = "${data.terraform_remote_state.bootstrap.location}"
  resource_group_name           = "${data.terraform_remote_state.bootstrap.resource_group_name}"
  ...
  ip_configuration {
    name                                    = "${var.project}-${var.ocpinfra_module_name}-ip${format("%02d", count.index)}"
...
resource "azurerm_virtual_machine" "infra" {
  name                             = "${var.project}-${var.ocpinfra_module_name}-${format("%02d", count.index)}"
  location                         = "${data.terraform_remote_state.bootstrap.location}"
  resource_group_name              = "${data.terraform_remote_state.bootstrap.resource_group_name}"
  ...
  tags {
    displayName = "${var.project}-${var.ocpinfra_module_name} VM"
    configlabel = "${data.terraform_remote_state.bootstrap.configlabel}"
  }

  os_profile {
    computer_name  = "${var.project}-${var.ocpinfra_module_name}-${format("%02d", count.index)}"
  ...
  storage_os_disk {
    name              = "${var.project}-${var.ocpinfra_module_name}-${format("%02d", count.index)}-osdisk"
    caching           = "ReadWrite"
  ...
  storage_data_disk {
    name              = "${var.project}-${var.ocpinfra_module_name}-${format("%02d", count.index)}-container-pool"
    caching           = "ReadWrite"
