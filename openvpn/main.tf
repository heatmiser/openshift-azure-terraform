...
locals {
  subnet_list = ["${data.terraform_remote_state.base.master_subnet_address_range}", "${data.terraform_remote_state.base.node_subnet_address_range}"]
}
...
resource "random_id" "openvpndns" {
  byte_length = 4
}

resource "azurerm_public_ip" "openvpn_pip" {
  name                         = "${var.project}openvpn${random_id.openvpndns.hex}"
  resource_group_name          = "${data.terraform_remote_state.base.resource_group_name}"
  location                     = "${data.terraform_remote_state.base.location}"
  public_ip_address_allocation = "Static"
  domain_name_label            = "${var.project}openvpn${random_id.openvpndns.hex}"
}

# ******* NETWORK INTERFACES ***********

resource "azurerm_network_interface" "openvpn_nic" {
  name                      = "${var.project}-openvpn-nic${count.index}"
  location                  = "${data.terraform_remote_state.base.location}"
  resource_group_name       = "${data.terraform_remote_state.base.resource_group_name}"
...
resource "azurerm_virtual_machine" "openvpn-rsa" {
  ...
  tags {
    displayName = "${var.project}-openvpn VM Creation"
    environment = "${var.project}-${var.environment}-${var.env_version}"
  }

  os_profile {
    computer_name  = "${var.project}-openvpn"
  ...
  storage_os_disk {
    name              = "${var.project}-openvpn-osdisk"
    caching           = "ReadWrite"
  ...
  storage_data_disk {
    name              = "${var.project}-openvpn-docker-pool"
    caching           = "ReadWrite"
    disk_size_gb      = "${var.os_disk_size}"
    create_option     = "Empty"
    lun               = 0
